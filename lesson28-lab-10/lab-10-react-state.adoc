...
Create a `Counter` component with the following _props_ type in a file `Counter.tsx`.
[source, typescript]
----
type CounterProps = {
  label: string;
...
Create an `InputBox` component with the following _props_ type in a file `InputBox.tsx`.
[source, typescript]
----
type InputBoxProps = {
  label: string;
