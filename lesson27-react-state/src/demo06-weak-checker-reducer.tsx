...
function WeakChecker() {
  ...
  function inputHandler(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    dispatch({ type: "input-change", pass: input });
    // Validate after state update
    const res: string | true = validatePassword(input);
    dispatch({ type: "validate", pass: input, res });
  }
    ...
    <div>
      ...
      <div>
        <div>
          ...
          />
          <button type="button" onClick={() => dispatch({ type: "toggle-visibility" })}>
        ...
        <button onClick={() => dispatch({ type: "submit" })} disabled={!!state.error || !state.pass}>
          Submit
        </button>
      ...
      {state.lastSubmittedPass && state.stage === "reset" && (
        <div>
          Last submitted: {state.lastSubmittedPass.replace(/./g, "•")}
        </div>
      )}
    </div>
  );
